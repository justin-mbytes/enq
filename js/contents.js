...
function( $      , coda_slider , jquery_easing , after_scroll , common){
	...
	//for mobile users, show select subject lightbox
	$(document).ready(function(){
		var window_width = $window.width();
		if(window_width == 426 || window_width > 425){
	...
	//close subject lightbox
	$("#math_btn, #english_btn, #physics_btn, #chemistry_btn, #biology_btn").click(function(){
		$('#select_subject').fadeOut(1000);
	});


	//detect initial curriculum is selected from dropdown box
	$("#grade_select").change(function(){
		$('#ok_btn').css('display','inline-table');
	});


	//close select curriculum lightbox (needs to be coded to show corrent content once lightbox is closed)
	$("#ok_btn").click(function(){
		$('#select_curriculum').fadeOut(1000);
		$('#contents_math').css('display','block');
	});
});